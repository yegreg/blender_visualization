...
def single_tree_path(starting_position, basis, directions, obj, shift):
    ...
    if len(directions) < N_LEVELS:
        new_position += np.random.uniform(-TREE_SIZE / 3, TREE_SIZE / 3, [3])
...
def full_tree_animation():
    ...
    for i in range(len(all_directions)):
        ...
        single_tree_path(starting_point, standard_basis, directions, emitter, 0)
